...
これまでPythonとDjangoを使用するバックエンド開発がメインでしたが、フロントエンドの知識には自信がありませんでした。個人的にあまり経験のないReact（TypeScript）を使用し、さらなる成長を目指して選定しました。
その過程で、フロントエンド側の知識が不足していることを改めて感じました。技術課題に取り組む前後で、自分が成長したことを実感しています。
特に良かった取り組みは、フロントエンドとバックエンドの両方でテストコードを作成したことです。これにより、作成したコードが無責任にならないように、きちんとした品質を維持することができました。
